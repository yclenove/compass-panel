...
class plugin(object):
    ...
    def_plugin_type = [
        {"title": "全部", "type": 0, "ps": ""},
        {"title": "已安装", "type": -1, "ps": ""},
        {"title": "运行环境", "type": 1, "ps": ""},
        {"title": "数据软件", "type": 2, "ps": ""},
        {"title": "代码管理", "type": 3, "ps": ""},
        {"title": "系统工具", "type": 4, "ps": ""},
        {"title": "其他插件", "type": 5, "ps": ""},
        {"title": "辅助插件", "type": 6, "ps": ""},
        {"title": "AI插件", "type": 7, "ps": ""},
    ]
                ...
                try:
                    ...
                    for index in range(len(data)):
                        if data[index]["coexist"]:
                            if (
                                data[index]["versions"] == plugin_ver
                                or plugin_ver in data[index]["versions"]
                            ):
        ...
        try:
            ...
            for pn in pn_list:
                ...
                cmd = "cd {0} && bash {1} install {2}".format(
                    mw.getPluginDir() + "/" + name, pluginInfo["shell"], version
                )
    ...
    def hookInstallOption(self, hook_name, info):
        ...
        for x in range(len(src_data)):
            if (
                src_data[x]["title"] == info["title"]
                and src_data[x]["name"] == info["name"]
            ):
                isNeedAdd = False
    ...
    def install(self, name, version, upgrade=None):
        if name.strip() == "":
        ...
        exec_bash = "cd {0} && bash {1} install {2}".format(
            mw.getPluginDir() + "/" + name, info_data["shell"], version
        )
    ...
    def makeCoexistData(self, data):
        plugins = []
        if isinstance(data["versions"], list) and "coexist" in data and data["coexist"]:
    ...
    def makeCoexistList(
        ...
        plugins_t = []
        # 返回指定类型
        if plugin_type is not None and data["pid"] == plugin_type:
            return self.makeCoexistData(data)

        # 全部
        if plugin_type is None or plugin_type == "0":
            return self.makeCoexistData(data)
        # 已经安装
    ...
    def checkStatusMThreadsByCache(self, info):
        try:
            self.__plugin_status_data = thisdb.getOptionByJson(
                self.__plugin_status_cachekey, default=None
            )
            threads = []
            ...
            thisdb.setOption(
                self.__plugin_status_cachekey, json.dumps(self.__plugin_status_data)
            )
    ...
    def getList(
        ...
        rdata["list"] = mw.getPage(
            {"count": data[1], "p": page, "tojs": "getSList", "row": size}
        )
        return rdata
    ...
    def updateZip(self, request_zip):
        ...
        if not os.path.exists(p_info):
            ...
            for df in os.walk(tmp_path):
                if len(df[2]) < 3:
                    continue
                if "info.json" not in df[2]:
                    continue
                if "install.sh" not in df[2]:
                    continue
    ...
    def callback(
        ...
        if mw.isDebugMode():
            print("callback:", script + "." + func, "args:", args)
